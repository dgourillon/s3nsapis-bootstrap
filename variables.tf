...
variable "prefix" {
  description = "Prefix used for resources that need unique names. Use 9 characters or less. - please include s3Ns: there"
  type        = string
  validation {
...
variable "short_prefix" {
  ...
  type        = string
  validation {
    condition     = try(length(var.short_prefix), 0) < 10
    error_message = "Use a maximum of 9 characters for short_prefix."
  }
}
